...
- Calls to OpenAI/HuggingFace/Telegram are not issued directly from the FastAPI
  layer. Instead, those external integrations are expected to live behind the
  upstream Cimeika API specified by `CIMEIKA_API`. Local development does not
  require those tokens beyond configuring the upstream endpoint.
